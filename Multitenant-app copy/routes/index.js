...
router.get('/store/:storeName', async (req, res) => {
  try {
    const storeName = req.params.storeName.trim().toLowerCase();

    console.log(`🛍️ Fetching store for: ${storeName}`);

    // Find tenant by name or slug (assuming you have a Tenant model)
    const tenant = await Tenant.findOne({
      $or: [
        { name: new RegExp(`^${storeName}$`, 'i') },
        { slug: new RegExp(`^${storeName}$`, 'i') }
      ]
    });

    if (!tenant) {
      console.warn(`⚠️ Store not found: ${storeName}`);
      return res.status(404).render('multitenant/store/not-found', {
        message: 'Store not found',
      });
    }

    const tenantId = tenant._id || tenant.tenantId;
    console.log(`✅ Found tenant ${tenant.name} (${tenantId})`);

    // Fetch all products for that tenant
    const products = await Product.find({ tenantId });

    console.log(`📦 Found ${products.length} products for ${tenant.name}`);

    // Render storefront page with tenant info & products
    res.render('multitenant/store/home', {
      tenant,
      products,
    ...
    res.status(500).render('multitenant/store/error', { message: 'Server error loading store.' });
  }
});
